from __future__ import print_functionimport platform,os
def tampil(x):
...
def simpan():
	...
	if len(id_friend) != 0:
		...
		try:
			...
			tampil('\rh[!]Saved successfully \rcMBFfriend.txt')
		except:
			tampil('\rm[!]Failed to save')
def open(d):
...
def crack(d):
	...
	if i.upper() == 'L':
		while 1:
			dir = inputD('[?]Please Enter address file')
			try:
		...
		for i in D:
			...
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Dear!Are you Not Satisfied with Results, Want to try again(y/n)',['Y','N'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
...
# \rh Congratulations you for completing the installation 
# \rhWelcome to the tool
# \rhBY\rp                                         PIRMANSX        \rk#
